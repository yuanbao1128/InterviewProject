...
import { StartPayload } from '../lib/schema';
...
r.post('/start-interview', async (c) => {
  ...
  const { rows: ir } = await query<{ id: string }>(
    `insert into app.interviews (user_id, title, role, style, duration_minutes, target_company, target_role, jd_text, resume_file_url, resume_summary, status, started_at)
     values (null, $1, $2, $3, $4, $5, $6, $7, $8, $9, 'ongoing', now()) returning id`,
    [
      `${payload.role} | ${payload.style} | ${payload.duration}分钟`,
      payload.role, payload.style, payload.duration,
  ...
  // 2) 生成问题清单（5-8题）
  const planningPrompt = await fs.readFile(path.join(process.cwd(), 'api/lib/prompts/planning.md'), 'utf-8');
  const sys = `${planningPrompt}\n请严格输出 JSON。上下文：\nJD:\n${payload.jdText || ''}\n简历要点:\n${JSON.stringify(payload.resumeSummary || {})}`;
  ...
  if (!Array.isArray(list) || list.length === 0) {
    ...
    list = Array.from({ length: 5 }).map((_, i) => ({
      order_no: i + 1,
      topic: i === 0 ? '自我介绍' : '经历深挖',
      text: i === 0 ? '请做一个简要自我介绍，并说明你为什么适合该岗位？' : `请详细说明一个你主导的项目（第${i}个），你的目标、动作、结果与复盘。`,
      intent: '基础面',
      rubric_ref: 'default'
    }));
  }
